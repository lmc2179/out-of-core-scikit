...
class OOCWrapper(object):
    ...
    def fit(self, data_type, data_source_dict, iterations=1):
        ...
        for i in range(iterations):
            ...
            self._fit_training_batches(input_batches, output_batches)

    def _fit_training_batches(self, input_batches, output_batches):
        [self.model.partial_fit(self._unpack_input_batch(i), self._unpack_output_batch(o)) for i, o in
         zip(input_batches, output_batches)]
    ...
    def predict(self, training_data_type, training_data_source_dict, target_data_type, target_data_source_dict):
        self._validate(training_data_source_dict, data_access.FIELDS_REQUIRED_FOR_TESTING_READ)
        self._validate(target_data_source_dict, data_access.FIELDS_REQUIRED_FOR_TESTING_WRITE)
        pk_batches, input_batches = self._get_test_batches(training_data_source_dict, training_data_type, self.input_fields)
        output_batches = self._predict_batches(input_batches)
        self._write_predictions(target_data_source_dict, target_data_type, output_batches, pk_batches)
